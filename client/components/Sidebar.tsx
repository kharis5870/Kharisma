      ...
      >
        <div className="p-4 h-[68px] flex items-center justify-between border-b border-gray-700">
          <div className={`flex items-center min-w-0 ${isExpanded ? '' : 'pointer-events-none'}`}>
            <span className={`font-bold text-2xl whitespace-nowrap transition-all duration-300 ${isExpanded ? 'ml-0 opacity-100' : 'w-0 opacity-0'}`}>
              Kharisma
            </span>
          </div>
        ...
        <div className="p-4 border-t border-gray-700 flex-shrink-0">
         ...
         <button onClick={() => setShowLogoutConfirm(true)} className={`flex items-center w-full p-3 rounded-lg text-left hover:bg-red-500`}>
